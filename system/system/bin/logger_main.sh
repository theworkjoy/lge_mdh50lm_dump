...
if [[ $file_cnt > 0 ]]; then
    if [[ $log_to_data_partition == 1 ]]; then
        ...
        while true; do
            /system/bin/logcat -v threadtime -b main *:V -f /data/logger/${log_file} -n $file_cnt -r $file_size_kb
        ...
        /system/bin/logcat -v threadtime -b main *:V -f /${tmp_log_path}/encryption_log/${log_file} -n $file_cnt -r $file_size_kb
    fi
fi
